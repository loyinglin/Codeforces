...
/************************** 题解 **********************
 ...
 如果维护一个dp[i]从大到小的单调队列，里面的a[i]+i不具备单调性；故而单调队列无解。
...
//    return 0;
    ...
    freopen("output_deque.txt", "w", stdout);
    int n;
    deque<Node> queue;
    while (scanf("%d", &n) != EOF) {
        for (int i = 1; i <= n; ++i) {
        ...
        d[1] = 0;
        queue.push_back(Node(d[1], 1 + a[1]));
            ...
            while (!queue.empty()) {
                ...
                if (top.pos < i) {
                    ...
                    while (!queue.empty()) {
                        ...
                        if (i + a[i] >= back.pos && d[i] <= back.value) {
                            queue.pop_back();
                        }
                        else {
                            break;
                    ...
                    if (d[i] > queue.back().value) {
                        queue.push_back(Node(d[i], a[i] + i));
                    }
                    break;
                }
