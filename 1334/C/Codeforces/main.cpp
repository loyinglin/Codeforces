 ...
 **题目解析：**
 位置i有多种可能爆炸顺序：i最先爆炸，i-1先爆炸然后i爆炸，i+1先爆炸然后i爆炸，i-1和i+1都爆炸然后i再爆炸；
 那么，我们用dp[i][j]来表示前i个全部爆炸的最小代价；
 dp[i][0] 表示第i个不做任何操作，从该状态转移出去，需要计算该位置爆炸的代价；
 这样太复杂了 ~！
 
 重新看了一遍题目，发现只能向右爆炸，那么先每一个点
 
 先考虑全部爆炸，再遍历枚举，每一个首先爆炸的代价；
