...
/************************** 题解 **********************
 题目链接：
 题目大意：
 有一个国家的地图，为n个节点的树，首都在节点1。
 m个旅行家从首都出发，终点为vis[i]（表示第vis[i]个节点），树的的边(u,v)表示一条节点u到v的路，经过的代价为边的权值；
 他们分别有各自的预算money[i]，如果路上经过的边的代价和大于预算，旅行家就不出发了；
  现在要求给每条边赋权值，要求最后收获的money最大；
